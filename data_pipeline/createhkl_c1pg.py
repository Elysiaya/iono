...
def main(year,output_dir,omni_file):
    output_file = os.path.join(output_dir, f"gim_{year}_hourlyaux_c1pg.hickle")
    ...
    for doy in range(1, days + 1):  # 1 到 365
        filename = f"c1pg{doy:03d}0.{str(year)[-2:]}i"
        filepath = f"C:\\Users\\zx\\Desktop\\毕业论文\\gim\\ionex_{year}_c1pg\\{filename}"
