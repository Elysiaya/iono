"""
Baseline 模型 (w/o FGL & w/o FiLM) 训练脚本
"""
...
from ablation_study.ablation_models import BaselineModel
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("ablation_train_baseline.log", encoding="utf-8"),
...
def build_dataloaders(config):
    ...
    logger.info(f"Baseline samples: {total}, Train: {train_size}, Val: {total - train_size}")
...
def train(resume_checkpoint=None):
    ...
    save_dir = config.ablation_dir / f"baseline_{timestamp}"
    save_dir.mkdir(parents=True, exist_ok=True)
    writer = SummaryWriter(log_dir=str(config.tensorboard_log_dir / 'baseline'))

    train_loader, val_loader = build_dataloaders(config)

    # 针对 Baseline，num_aux 为 0
    num_aux = 0
    model = BaselineModel(
    ...
    logger.info("=" * 80)
    logger.info("Training Ablation Model: Baseline (w/o Both)")
    logger.info("=" * 80)
    ...
    for epoch in range(start_epoch, config.num_epochs):
        ...
        name = f"baseline_epoch{epoch+1:02d}_valRMSE{val_rmse:.4f}.pth"
        torch.save(checkpoint, save_dir / name)

        if avg_val_pred < best_val_loss:
            best_val_loss = avg_val_pred
            torch.save(checkpoint, save_dir / f"best_baseline.pth")
            patience_counter = 0
        else:
            patience_counter += 1
        ...
        logger.info(f"[Baseline] Epoch [{epoch+1}/{config.num_epochs}] Train RMSE: {train_rmse:.4f} | Val RMSE: {val_rmse:.4f}")
