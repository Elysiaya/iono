...
def build_dataloaders(hickle_paths, window_size, future_size, pred_steps, batch_size):
    ...
    train_loader = DataLoader(
        ...
        shuffle=True, num_workers=Config.num_workers, pin_memory=Config.pin_memory, prefetch_factor=2
    )
    
    val_loader = DataLoader(
        val_dataset, batch_size=batch_size,
        shuffle=False, num_workers=Config.num_workers, pin_memory=Config.pin_memory, prefetch_factor=2
    )

    logger.info(f"Total samples: {total}, Train: {train_size}, Val: {total - train_size}")
